...
export default function Home() {
  ...
  async function handleBuild() {
    ...
    try {
      ...
      if (fileObj) {
        ...
        // 1. Upload Document AND Analyze (Atomic Route - Standard FormData)
        ...
        let uploadRes;
        try {
          uploadRes = await fetch(`${getApiBaseUrl()}/document/upload_and_analyze`, {
            method: "POST",
            body: formData,
  ...
  function exportJSON() {
    const exportData = {
      ...
      evidenceReview: {
        product: twin.product_id,
        attribute: activeEvidence?.attribute,
        resolvedValue: activeEvidence?.resolvedValue,
        confidence: activeEvidence?.confidence,
        conflictDetected: activeEvidence?.conflict,
            ...
            <div className="relative border border-line bg-panel/80 p-6 backdrop-blur-sm sm:p-8">
              ...
              <div className="mt-6 flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="sm:text-right">
                  ...
                  <p className="mt-1 font-mono text-sm text-teal">
                    {activeEvidence?.attribute?.replace(/_/g, ' ')}
                  </p>
                </div>
              </div>
                ...
                <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
                  <div className="flex items-center gap-4">
                    <span className="font-display text-5xl font-semibold text-ivory">
                      {activeEvidence?.resolvedValue}
                    </span>

                    {activeEvidence?.conflict && (
                  ...
                  <div className="min-w-[10rem]">
                    <div className="flex items-center justify-between font-mono text-[10px] tracking-wide text-mist uppercase">
                      ...
                      <span className="text-ivory">
                        {Math.round((activeEvidence?.confidence || 0) * 100)}%
                      </span>
                    </div>
                    ...
                    <div className="mt-1.5 h-1.5 w-full overflow-hidden rounded-full bg-panel">
                      <div
                        ...
                        style={{
                          width: `${(activeEvidence?.confidence || 0) * 100}%`,
                        }}
                      />
                    </div>
                  </div>
              ...
              <div className="mt-8">
                ...
                <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-3">
                  {(activeEvidence?.sources || []).map((s: any, i: number) => (
                    <div
                      key={s.name}
              ...
              <div className="mt-8 rounded-md border border-line bg-panel-2 p-4">
                ...
                <p className="mt-3 text-sm text-ivory">
                  {activeEvidence?.conflict ? (
                    <>
                      Conflict detected between sources. {activeEvidence?.analysis}
                    </>
                  ) : (
                    "All sources agree on this attribute value."
                  )}
                </p>

                <p className="mt-2 text-sm text-mist">
                  {/* Additional analysis text could go here */}
                </p>
              </div>
                      ...
                      onClick={async () => {
                        ...
                        try {
                            await fetch(`${getApiBaseUrl()}/review/${twin.product_id}`, {
                              ...
                              body: JSON.stringify({ 
                                attribute_name: activeEvidence.attribute, 
                                approved_value: activeEvidence.resolvedValue,
                                approved_unit: activeEvidence.unit,
                                status: "verified",
                                notes: reviewNotes 
                              })
                            });
                ...
                <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                  {Object.entries(twin.attributes).map(([key, attr]: [string, any]) => (
                    <div
                      key={key}
                      onClick={() => {
                        const conflictForAttr = twin.conflicts?.find((c: any) => c.attribute_name === key);
                        const sources = [];
                        if (conflictForAttr) {
                          sources.push({
                            name: conflictForAttr.source_1,
                            detail: "Conflict source 1",
                            value: conflictForAttr.value_1,
                            trust: "Medium",
                            agrees: false
                          });
                          sources.push({
                            name: conflictForAttr.source_2,
                            detail: "Conflict source 2",
                            value: conflictForAttr.value_2,
                            trust: "Medium",
                            agrees: false
                          });
                        }
                        if (attr.evidence) {
                          attr.evidence.forEach((e: any) => {
                            sources.push({
                              name: e.source,
                              detail: e.page ? `Page ${e.page}` : (e.snippet ? 'Snippet' : 'Extracted'),
                              value: attr.value,
                              trust: "High",
                              agrees: true
                            });
                        ...
                        setActiveEvidence({
                          product: twin.product_id,
                          attribute: key,
                          resolvedValue: attr.value,
                          unit: attr.unit,
                          confidence: attr.confidence,
                          status: attr.status,
                          conflict: !!conflictForAttr,
                          analysis: conflictForAttr?.description || "",
                          sources: sources
                        });
                        setBuildState("evidence");
                      }}
                      className="cursor-pointer rounded-md border border-line bg-panel-2 px-4 py-3 transition hover:border-teal/50 hover:bg-panel-2/80"
                    >
                      <div className="flex items-center justify-between">
                        <p className="font-mono text-[10px] tracking-wide text-mist uppercase">
                          {key.replace(/_/g, ' ')}
