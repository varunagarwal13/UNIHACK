...
def get_product(id: str, db: Session = Depends(get_db)):
    ...
    # Calculate overall confidence (average of attribute confidences, scaled to 0-100)
    avg_conf = 0
    if product.attributes:
        total_conf = sum(attr.confidence for attr in product.attributes)
        avg_conf = int((total_conf / len(product.attributes)) * 100)
